    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                ...
                # for h in range(224):
                #     for w in range(224):
                #         print(prediction[h,w])
                # print('this is if', prediction)
...
#             plt.savefig("/root/sav/{}".format(case) +'.jpg',dpi=800)
    ...
    #     # 保存预测结果的路径
    #     result_path = os.path.join(test_save_path, "visualization")
    #     os.makedirs(result_path, exist_ok=True)
    #
    #     # 保存原始图像
    #     plt.imshow(image.squeeze(), cmap='gray')
    #     plt.title("Original Image")
    #     plt.savefig(os.path.join(result_path, case + "_original.png"))
    #     plt.close()
    #
    #     # 保存预测图像
    #     plt.imshow(prediction.squeeze(), cmap='gray')
    #     plt.title("Predicted Mask")
    #     plt.savefig(os.path.join(result_path, case + "_predicted.png"))
    #     plt.close()
    #
    #     # 保存标签图像
    #     plt.imshow(label.squeeze(), cmap='gray')
    #     plt.title("Ground Truth Mask")
    #     plt.savefig(os.path.join(result_path, case + "_ground_truth.png"))
    #     plt.close()
    #
    # return metric_list
